...
namespace Hos.ScheduleMaster.QuartzHost.HosLock
{
    public class DatabaseLock : IHosLock
    {
        ...
        {
            _key = key;
            _isLocked = _dbContext.Database.ExecuteSqlRaw($"update schedulelocks set status=1,lockedtime=getdate(),lockednode='{ConfigurationCache.NodeSetting.IdentityName}' where scheduleid='{key}' and status=0") > 0;
            return _isLocked;
        }
    }
}
